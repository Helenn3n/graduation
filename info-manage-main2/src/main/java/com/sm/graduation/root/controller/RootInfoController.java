...
public class RootInfoController {
    ...
    /** 测试 */
    @RequestMapping("/test")
    public AjaxResult test(@RequestParam(defaultValue = "1", value = "page") Integer pageNum,
                              @RequestParam(defaultValue = "10", value = "limit") Integer pageSize,
                              @RequestParam(defaultValue = ""  , value = "year") String usr
    ){
        HashMap<String, Object> map = new HashMap<>();
        ...
        if (usr == null || usr.equals("")) {
            map.put("usr","");
            List<Test> tests = testService.listAll(map);
            PageInfo<Test> page = new PageInfo<>(tests);
    ...
    public AjaxResult addTest(Test test){
    ...
    public AjaxResult delTest(@RequestParam(value = "id") Integer id){
        checkInService.delete(id);
        return success("删除成功");
    }

    @RequestMapping("/batchDelTest")
    public AjaxResult batchDelTest(String listStr){
        if (null != listStr && !"".equals(listStr)){
            String[] ids = listStr.split(",");
            for (String id:ids) {
                if (null != id && !"".equals(id)) {
